...
import { GroundIcon } from "../icons/SldIcons";
import useSldStore from "../../store/useSldStore";

const GroundNode = memo(({ id, data, selected }) => {
  ...
  return (
    ...
    >
      <Handle type="target" position={Position.Top} style={{ background: "#6b7280", width: 10, height: 10 }} />
      <GroundIcon size={40} />
      <div style={{ fontSize: "11px", fontWeight: 600, color: "#1f2937", marginTop: 4 }}>{data.name}</div>
      <div style={{ fontSize: "9px", color: "#6b7280" }}>{data.groundType}</div>
    </div>
  );
});
