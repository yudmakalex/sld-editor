...
import { GeneratorIcon } from "../icons/SldIcons";
import useSldStore from "../../store/useSldStore";

const GeneratorNode = memo(({ id, data, selected }) => {
  ...
  return (
    ...
    >
      ...
      <GeneratorIcon size={40} status={data.status} />
      <div style={{ fontSize: "11px", fontWeight: 600, color: "#1f2937", marginTop: 4 }}>{data.name}</div>
      <div style={{ fontSize: "9px", color: "#6b7280" }}>{data.power} MW</div>
